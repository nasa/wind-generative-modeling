...
real = pd.read_csv('scripts/postprocessing/speed_cond/real.csv')
...
gmm_1 = pd.read_csv('scripts/postprocessing/speed_cond/gmm_generated_data_uv_2.23_6542.csv', index_col = [0])
gmm_1['macro_ws_str'] = '(-0.001, 2.235]'

gmm_2 = pd.read_csv('scripts/postprocessing/speed_cond/gmm_generated_data_uv_5.36_6542.csv', index_col = [0])
gmm_2['macro_ws_str'] = '(2.235, 5.364]'

gmm_3 = pd.read_csv('scripts/postprocessing/speed_cond/gmm_generated_data_uv_8.05_6542.csv', index_col = [0])
gmm_3['macro_ws_str'] = '(5.364, 8.047]'

gmm_4 = pd.read_csv('scripts/postprocessing/speed_cond/gmm_generated_data_uv_15.65_6542.csv', index_col = [0])
...
for (j, m) in enumerate(models):
    ax[j, 0].text(-0.2, 0.5, m, transform = ax[j, 0].transAxes, rotation = 90, va = 'center', ha = 'center', weight = 'bold')
...
for (j, m) in enumerate(models):
    ...
    for (i, macro_speed) in enumerate(speeds):
        ...
        ax[j, i].plot(ALTITUDES, mean_real_speed, label = 'Real', color = 'blue')
        ax[j, i].fill_between(ALTITUDES, mean_real_speed - std_real_speed, mean_real_speed + std_real_speed, alpha = 0.2, color = 'blue')
        
        ax[j, i].plot(ALTITUDES, mean_pred_speed, label = 'Generated', color = 'orange')
        ax[j, i].fill_between(ALTITUDES, mean_pred_speed - std_pred_speed, mean_pred_speed + std_pred_speed, alpha = 0.2, color = 'orange') 
        ax[j, i].legend()
