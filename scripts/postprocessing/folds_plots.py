...
'''
real = pd.read_csv('scripts/postprocessing/real.csv')
speed_folds = ['(-0.001, 2.235]', '(2.235, 5.364]', '(5.364, 8.047]', '(8.047, 15.646]']
...
real_fold = real[(real['macro_ws_str'] == withhold_speed) & (real['macro_wd_str'] == withhold_direction)]

ddpm = pd.read_csv(f'scripts/postprocessing/folds/ddpm/{withhold_speed}-{withhold_direction}.csv')
ddpm = ddpm[(ddpm['macro_ws_str'] == withhold_speed) & (ddpm['macro_wd_str'] == withhold_direction)]

fm = pd.read_csv(f'scripts/postprocessing/folds/fm/{withhold_speed}-{withhold_direction}.csv')
fm = fm[(fm['macro_ws_str'] == withhold_speed) & (fm['macro_wd_str'] == withhold_direction)]

gmm = pd.read_csv(f'scripts/postprocessing/folds/gmm/{withhold_direction}_{withhold_speed}.csv')
...
ax[0].text(15.0, 20.0, 'KLD = ' + str(round(kl_divergence(uv, uv_gmm) + kl_divergence(uv_gmm, uv), 2)))
...
ax[1].text(15.0, 20.0, 'KLD = ' + str(round(kl_divergence(uv, uv_ddpm) + kl_divergence(uv_ddpm, uv), 2)))
...
ax[2].text(15.0, 20.0, 'KLD = ' + str(round(kl_divergence(uv, uv_fm) + kl_divergence(uv_fm, uv), 2)))
fig.tight_layout()
...
GMM on each fold.
'''
fig, ax = plt.subplots(len(speed_folds), len(direction_folds), figsize = (10, 10), sharex = True, sharey = True)
